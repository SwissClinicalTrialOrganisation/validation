...
## Packages that currently have no tests

The `test_skeleton` function can be used to create the relevant folder structure for testing a new package. In the code below, substitute `pkg` with the name of the package to be tested and add the names of the function(s) you want to test in the `fun` argument. 

```{r, eval = FALSE}
test_skeleton("pkg", fun = c("fun", "fun2", "etc"))
```
...
- `setup-pkg.R` is for any necessary setup code,
- for each `fun` in the `fun` argument, a `test-function.R` file is created, which will contain the actual testing code.
...
We specify download = FALSE because validation will by default download the files from GitHub and run those tests.
